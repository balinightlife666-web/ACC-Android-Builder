# LOST & FOUND: NIGHT SHIFT — M6-B Progression Table v1

Status: LOCK CANDIDATE / IMPLEMENTATION BRANCH
Scope: Unlock & Career Tiers only

| Shift | Existing Title | Career Tier | M6-B Reward |
|---|---|---|---|
| 1 | PETUGAS BARU | ORIENTASI | Akses shift inti |
| 2 | OPERATOR JUNIOR | OPERASIONAL | Skin Industrial Shift |
| 3 | OPERATOR SHIFT | OPERASIONAL | Skin Retro Airport |
| 4 | OPERATOR SENIOR | SENIOR | Skin Black Ops |
| 5 | SPESIALIS KASUS | SPESIALIS | Akses pembelian Army Field |
| 6 | PENGAWAS MALAM | PENGAWAS MALAM | Promosi karier: Pengawas Malam |
| 7 | ANALIS INSIDEN | SPESIALIS MALAM | Akses pembelian Sakura Night |
| 8 | SPESIALIS ARSIP | ARSIP ELIT | Promosi karier: Arsip Elit |
| 9 | PEMIMPIN SHIFT | KOMANDO SHIFT | Akses pembelian Street Graffiti |
| 10 | MASTER NIGHT SHIFT | MASTER NIGHT SHIFT | Promosi karier: Master Night Shift |

## Hard locks

- Existing XP and M6-A thresholds remain canonical and unchanged.
- No second XP currency or new progression DataStore.
- Existing Config.Rewards values remain unchanged.
- Credits values and station theme prices remain unchanged.
- Collectible drop rarity remains unchanged.
- Core tutorial / first-session loop remains available from Shift Level 1.
- Career gates are cosmetic/prestige gates only in M6-B v1.
- Previously-owned station skins are grandfathered and remain equip-safe regardless of the new acquisition gate.
- No case canon, serialized collectible, trading, showcase lifecycle, or decision-color semantics are changed.

## Station acquisition gates

- STANDARD_OPS: Shift 1
- INDUSTRIAL_SHIFT: Shift 2
- RETRO_AIRPORT: Shift 3
- BLACK_OPS: Shift 4
- ARMY_FIELD: Shift 5
- SAKURA_NIGHT: Shift 7
- STREET_GRAFFITI: Shift 9

## Career presentation

M6-B adds a compact career bar below the existing M6-A progression HUD. Career promotions at Shift 6, 8, and 10 are presentation/prestige rewards only and do not grant gameplay power.

Deferred event/Robux catalog remains outside M6-B v1.
